...
print_maze();
    ...
    // valid_positions.pop();
		...
		if(walk({pos.i,pos.j+1})){//direita
			return true;
		}
		if(walk({pos.i,pos.j-1})){//esquerda
			return true;
		}
		if(walk({pos.i-1,pos.j})){//cima
			return true;
		}
		if(walk({pos.i+1,pos.j})){//baixo
			return true;
		}
		// maze[pos.i][pos.j]='.';
	

		return false;//sida não encontrada
}
...
int main(int argc, char* argv[]) {
	...
	if(exit_found) cout <<"Existe uma saida valida!";
